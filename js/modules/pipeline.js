    ...
    loadBoard: async (searchTerm = '') => {
        ...
        try {
            ...
            container.innerHTML = stages.map(stage => {
                ...
                items.forEach(item => {
                    if (item.currency === 'USD') totalUSD += (item.value || 0);
                    else if (item.currency === 'BOB') totalBOB += (item.value || 0);
                });

                // Formateamos el texto del total (Ej: "$1,000 / Bs 500")
                let totalText = '';
                if (totalUSD > 0) totalText += `<span style="color:#111;">${Formatters.toCurrency(totalUSD, 'USD')}</span>`;
                if (totalUSD > 0 && totalBOB > 0) totalText += ` <span style="color:#CCC;">|</span> `;
                if (totalBOB > 0) totalText += `<span style="color:#111;">${Formatters.toCurrency(totalBOB, 'BOB')}</span>`;
                if (totalUSD === 0 && totalBOB === 0) totalText = '<span style="color:#999;">-</span>';
                // -------------------------------------
